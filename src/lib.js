...
export function publicShell(route, content) {
  return `<a class="skip" href="#main">Skip to content</a><header class="public-header"><a href="/">${brand(true)}</a><nav>${publicNav(route)}</nav><div class="header-actions"><button class="quiet" data-command>${icon('search')} Search</button>${button('Enterprise access →','/app/','ghost')}</div><button class="menu" data-menu aria-label="Open navigation" aria-controls="mobile-nav" aria-expanded="false">${icon('menu')}</button></header><nav id="mobile-nav" class="mobile-nav" data-mobile-nav hidden>${publicNav(route)}<a href="/app/">Enterprise access</a></nav><main id="main">${content}</main>${publicFooter()}${overlay()}<div class="toast" data-toast role="status" aria-live="polite" hidden></div>`;
}
