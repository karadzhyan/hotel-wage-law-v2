...
- Protection state: anonymous preview access redirected to Vercel login under the team's existing Vercel Authentication policy. Verification used Vercel's automation-bypass path and the documented toolbar-suppression header for end-to-end tests, without disabling or weakening preview protection. Protected remote runs infer their route prefix from the base URL and disable persistent HTML-report and trace output so the request header is not retained in test artifacts. No bypass value is stored in the repository or this evidence record.
